...
/**
 * ⚙️ Dev-only mock fallback (Vite/HMR únicamente).
 *
 *  - El API real `api-production.finky.la` no devuelve configuraciones para
 *    los slugs de prueba en preview/local, por lo que esta capa se utiliza
 *    para verificar el rediseño visualmente sin depender del backend.
 *  - Se registra SÓLO cuando `environment.production === false` (ver
 *    app.config.ts). En el build de producción este archivo simplemente no
 *    se enlaza al árbol de providers.
 *
 *  Para deshabilitar del todo: borrar este archivo y la línea de
 *  `mockConfigInterceptor` dentro de `app.config.ts`.
 */
...
const mocks: Record<string, LandingConfig> = {
  ...
  ibero: {
    plantilla: 2,
    colores: { oscuro: deepNavy, claro: finkyOrange },
    nombreUniversidad: 'Iberoamericana',
    idUniversidad: '3',
    iconUniversity: '/assets/images/universities/ibero.png',
    banner: {
      pc: '/assets/images/iberoFondo.jpg',
      movil: '/assets/images/iberoFondo.jpg',
      titulo: 'Estudiar en la Ibero es fácil con Finky.',
      contenido:
        'Concéntrate en estudiar. Nosotros nos encargamos de la financiación: 100% online, sin trámites eternos y con cuotas a tu medida.',
    },
    bullets: [
      { titulo: 'Solicitud rápida y online', desc: 'Aprobación 100% digital.' },
      { titulo: 'Sin trámites eternos', desc: 'Solo lo esencial.' },
      { titulo: 'Cuotas a tu medida', desc: 'Tú eliges el plan.' },
      { titulo: 'Aprobación en 5 min', desc: 'Respuesta inmediata.' },
    ],
    oferta: baseOferta,
    prompt_version: 'finky',
  },
